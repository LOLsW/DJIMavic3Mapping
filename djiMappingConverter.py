...
if __name__ == "__main__":
    ...
    shape_file = input("Drag and drop shape .kml file: ").replace("\"", "").replace("'", "") # Remove quotes and double quotes
    # check if input starts with "&", Windows does this when dropping files whose path contains spaces (it also encloses the path in quotes)
    if shape_file.startswith("&"):
    ...
    # Wait for user input to close terminal, otherwise the terminal would close immediately not allowing the user to read the file path
